...
# VISION_ENCODER_PATH = {}
try:
    # Keep vision encoder checkpoint paths consistent with the simplified (hymm) codebase.
    # This is required by post-train codepaths that construct VisionEncoder without an explicit path.
    from hymm.constants import VISION_ENCODER_PATH as _HYMM_VISION_ENCODER_PATH
except Exception:
    _HYMM_VISION_ENCODER_PATH = {}

# NOTE: keep as a plain dict for serialization/debug printing.
VISION_ENCODER_PATH = dict(_HYMM_VISION_ENCODER_PATH) if _HYMM_VISION_ENCODER_PATH else {}
