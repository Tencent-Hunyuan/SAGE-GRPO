...
# Global paths for MultilingualPromptFormat, set when _load_byt5 runs (used by hunyuan_video_grpo_pipeline)
_MULTILINGUAL_PROMPT_FORMAT_FONT_PATH = None
_MULTILINGUAL_PROMPT_FORMAT_COLOR_PATH = None
...
class HunyuanVideo_1_5_Pipeline(DiffusionPipeline):
    ...
    def _load_byt5(cls, cached_folder, glyph_byT5_v2, byt5_max_length, device):
        ...
        try:
            ...
            globals()["_MULTILINGUAL_PROMPT_FORMAT_FONT_PATH"] = multilingual_prompt_format_font_path
            globals()["_MULTILINGUAL_PROMPT_FORMAT_COLOR_PATH"] = multilingual_prompt_format_color_path
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            # `torch.randn(..., generator=gen, device=init_device)` requires the generator device type
            # to match the target `device` argument. We default to CPU noise init for peak-memory reasons,
            # but some callsites may construct CUDA generators (e.g. `torch.Generator(device="cuda")`).
            #
            # To keep compatibility, we mirror the RNG seed onto a generator on `noise_init_device`.
            init_device = self.noise_init_device

            def _to_device_generator(gen: torch.Generator, target_device: torch.device) -> torch.Generator:
                try:
                    gen_dev = gen.device
                except Exception:
                    # Older torch may not expose `.device`; fall back to using original generator.
                    return gen
                if torch.device(gen_dev).type == torch.device(target_device).type:
                    return gen
                # Re-create generator on the target device with the same initial seed.
                # NOTE: this preserves determinism for the common "manual_seed" usage pattern.
                return torch.Generator(device=target_device).manual_seed(gen.initial_seed())

            if isinstance(generator, list):
                generator = [_to_device_generator(g, init_device) for g in generator]
            elif isinstance(generator, torch.Generator):
                generator = _to_device_generator(generator, init_device)

            latents = torch.randn(shape, generator=generator, device=init_device, dtype=dtype).to(device)
        else:
    ...
    def _prepare_byt5_embeddings(self, prompts, device):
        ...
        # Ensure byT5 components and prompt_format are initialized.
        if getattr(self, "byt5_model", None) is None or getattr(self, "byt5_tokenizer", None) is None:
            return {}

        if getattr(self, "prompt_format", None) is None:
            ...
            if font_path and color_path:
                self.prompt_format = MultilingualPromptFormat(font_path=font_path, color_path=color_path)
            else:
                # Glyph assets are not available; skip byT5 processing.
                return {}
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if reference_image is not None:
            ...
            if height is not None and width is not None:
                ...
                if height <= 0 or width <= 0:
                    raise ValueError(f"height/width must be positive integers, got {height}x{width}")
            elif self.ideal_resolution is not None:
        ...
        if get_rank() == 0:
            shown_aspect_ratio = f"{width}:{height}" if task_type == "t2v" else f"{width}:{height}"
            print(
                '\n'
                f"{'=' * 60}\n"
                ...
                f"Aspect Ratio:              {shown_aspect_ratio}\n"
    ...
    def create_pipeline(
        ...
        **kwargs,
        ...
        ref_model = None
        if create_ref_model:
            ref_model = HunyuanVideo_1_5_DiffusionTransformer.from_pretrained(
                **from_pretrain_kwargs,
                torch_dtype=transformer_dtype,
                low_cpu_mem_usage=True,
            ).to(transformer_init_device)
